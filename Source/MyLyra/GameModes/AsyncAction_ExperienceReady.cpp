...
void UAsyncAction_ExperienceReady::Activate()
{
	...
	{
		// 현재 AsyncAction 대상이었던 World가 더 디상 Valid하지 않으므로, 제거 진행
		// - GameInstance에서 레퍼런스 대상으로 제거시킴
		// - UAsyncAction의 캐싱 위치가 GameInstance임을 알 수 있음
		SetReadyToDestroy();
	}
}
...
void UAsyncAction_ExperienceReady::Step2_ListenToExperienceLoading(AGameStateBase* GameState)
{
	...
	else
	{
		...
		ExperienceManagerComponent->CallOrRegister_OnExperienceLoaded(FOnMyLyraExperienceLoaded::FDelegate::CreateUObject(this, &ThisClass::Step3_HandleExperienceLoaded));
	}
}

void UAsyncAction_ExperienceReady::Step3_HandleExperienceLoaded(const UMyLyraExperienceDefinition* CurrentExperience)
{
